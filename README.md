...
| Evaluation protocol | `docs/`, `benchmarks/evaluation/`, `benchmarks/corpus/` | Issue #18: written methodology plus an executable protocol over a fixed, checksummed replay corpus (`make eval-offline`, offline, no credentials). It records provider/model/version/config, keeps measured results apart from subjective notes, and hard-codes no winner. Live-provider comparison is a separate, billable, explicitly confirmed step. See [docs/evaluation-methodology.md](docs/evaluation-methodology.md). |
