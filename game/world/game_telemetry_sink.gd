...
const EVENT_GENERATION_SENT := "generation.sent"
const EVENT_GENERATION_RESPONSE_RECEIVED := "generation.response_received"
...
const KNOWN_EVENTS: PackedStringArray = [
	...
	EVENT_GENERATION_SENT,
	EVENT_GENERATION_RESPONSE_RECEIVED,
...
# Allowed attributes per event (mirroring PR29 EVENT_ATTRIBUTE_ALLOWLIST)
const COMMON_ATTRIBUTES: PackedStringArray = ["shadow_comparison_id", "replay_id", "execution_mode"]
...
const EVENT_ALLOWLIST := {
	...
	EVENT_GENERATION_QUEUED: ["frontier_id", "depth", "exit_direction", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_GENERATION_SENT: ["frontier_id", "depth", "exit_direction", "queue_ms", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_GENERATION_RESPONSE_RECEIVED: ["provider", "model", "network_ms", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_GENERATION_ACCEPTED: ["provider", "model", "room_type", "room_size", "danger", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_GENERATION_NORMALIZED: ["provider", "model", "room_type", "room_size", "danger", "normalize_reason", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_GENERATION_REJECTED: ["reject_reason", "provider", "model", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_GENERATION_FALLBACK_APPLIED: ["fallback_reason", "provider", "model", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_ROOM_COMMITTED: ["room_id", "frontier_id", "room_type", "room_size", "danger", "exit_count", "has_secret", "enemy_density", "loot_density", "materialization_ms", "provider", "model", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_DOOR_REVEALED: ["room_id", "time_to_visible_ms", "shadow_comparison_id", "replay_id", "execution_mode"],
	EVENT_ROOM_ENTERED: ["room_id", "time_to_entry_ms", "provider", "model", "shadow_comparison_id", "replay_id", "execution_mode"],
}

# Bounded enum sets for scalar validation (mirroring PR29 ATTRIBUTE_REGISTRY)
...
func _update_enabled_flag() -> void:
	var env_opt := OS.get_environment("DUNGEON_TELEMETRY_ENABLED").strip_edges().to_lower()
	if env_opt in ["1", "true", "yes", "on"]:
		enabled = true
	else:
		enabled = false
...
func enqueue_event(
	...
	if request_id != null and str(request_id) != "":
		...
		# request_id can only be null for frontier.discovered and generation.queued
		if event_name != EVENT_FRONTIER_DISCOVERED and event_name != EVENT_GENERATION_QUEUED:
			dropped_invalid += 1
			dropped_total += 1
			return false
...
static func _sanitize_attribute_value(key: String, val: Variant) -> Variant:
	# Scalar check: only bool, int, float, String allowed
	if val is bool:
		if key == "has_secret":
			return val
		return null
	...
	if val is int:
		...
		if key == "exit_count":
			return clampi(val, 0, 8)
		# Durations/timings can also be provided as ints
		if key in ["queue_ms", "network_ms", "materialization_ms", "time_to_visible_ms", "time_to_entry_ms"]:
			if val < 0 or val > 3600000:
				return null
			return float(val)
		if key in ["enemy_density", "loot_density"]:
			return clampf(float(val), 0.0, 1.0)
		return null
	...
	if val is float:
		if not is_finite(val):
			return null
		if key in ["queue_ms", "network_ms", "materialization_ms", "time_to_visible_ms", "time_to_entry_ms"]:
			if val < 0.0 or val > 3600000.0:
				return null
			return snappedf(val, 0.001)
		if key in ["enemy_density", "loot_density"]:
			if val < 0.0 or val > 1.0:
				return null
			return snappedf(val, 0.0001)
		return null
	...
	if val is String:
		...
		if s.contains("\n") or s.contains("\r"):
			return null
		# Ban secret-like text and URL schemes
		var lower_s := s.to_lower()
		if lower_s.contains("bearer ") or lower_s.contains("sk-") or lower_s.contains("://"):
			return null
		...
		match key:
			...
			"normalize_reason":
				return s if ALLOWED_NORMALIZE_REASONS.has(s) else null
			"reject_reason":
				return s if ALLOWED_REJECT_REASONS.has(s) else null
			"fallback_reason":
				return s if ALLOWED_FALLBACK_REASONS.has(s) else null
			"frontier_id":
				return s if _is_valid_frontier_id(s) else null
			"room_id", "shadow_comparison_id", "replay_id":
				return s if _is_valid_id(s) else null
			"provider", "model":
				return s if _is_valid_provider_model(s) else null
			_:
				return null
...
static func _is_valid_id(text: String) -> bool:
	...
	for i in range(1, text.length()):
		var c := text[i]
		if not (_is_alnum(c) or c == "_" or c == "." or c == "-"):
			return false
	return true


static func _is_valid_frontier_id(text: String) -> bool:
	...
	for i in range(1, text.length()):
		var c := text[i]
		if not (_is_alnum(c) or c == "_" or c == "." or c == "-" or c == ":"):
			return false
	return true


static func _is_valid_provider_model(text: String) -> bool:
	if text.length() < 1 or text.length() > MAX_STRING_LENGTH:
		return false
	# Must match MODEL_ID_RE: ^[A-Za-z0-9@][A-Za-z0-9_.:/@-]{0,127}$
	var first := text[0]
	if not (_is_alnum(first) or first == "@"):
		return false
	for i in range(1, text.length()):
		var c := text[i]
		if not (_is_alnum(c) or c == "_" or c == "." or c == ":" or c == "/" or c == "@" or c == "-"):
			return false
	return true
...
func _flush_batch() -> void:
	...
	# Ensure batch payload fits within MAX_BODY_BYTES (65536 bytes)
	...
	while count > 0:
		...
		if json_payload.to_utf8_buffer().size() <= MAX_BODY_BYTES:
			fits = true
			break
		# If single event exceeds MAX_BODY_BYTES, drop it
		if count == 1:
			_queue.pop_front()
			dropped_payload_limit += 1
			dropped_total += 1
			return
		# Otherwise reduce batch size to fit within byte limit
		count = count / 2

	if not fits or count == 0:
		return

	var final_payload_dict := {
		"schema_version": SCHEMA_VERSION,
		"events": batch_events,
	}
	var final_json := JSON.stringify(final_payload_dict)
	...
	_dispatch_http(final_json, count)
